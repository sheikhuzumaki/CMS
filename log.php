...
if (isset($_GET['Sub_teacher'])) {
  ...
  $usr_teacher = $_GET['usr_teacher'];
  $pass_teacher = $_GET['pass_teacher'];

  if (empty($usr_teacher) || empty($pass_teacher)) {
    echo '<script type="text/javascript">alert("Fill All The Blanks")</script>';
  }else{
    $tquery = "SELECT * FROM teacher WHERE usr_name = '$usr_teacher' && pass = '$pass_teacher' ";
    $tresult = mysqli_query($conn , $tquery);
    $tlog = mysqli_num_rows($tresult);
    if ($tlog == 1) {
      echo '<script type="text/javascript">alert("Teacher Found")</script>';
    }else{
...
if (isset($_GET['Sub_admin'])) {
  ...
  if (empty($usr_admin) || empty($pass_admin)) {
    ...
    if ($alog == 1) {
      echo '<script type="text/javascript">alert("Admin Found")</script>';
    }else{
      echo '<script type="text/javascript">alert("User Name or Password Incoorect !")</script>';
    }
  }
...
if (isset($_GET['Sub_student'])) {
  ...
  $usr_student = $_GET['usr_student'];
  $pass_student = $_GET['pass_student'];

  if (empty($usr_student) || empty($pass_student)) {
    echo '<script type="text/javascript">alert("Fill All The Blanks")</script>';
  }else{
    $squery = "SELECT * FROM student WHERE usr_name = '$usr_student' && pass = '$pass_student' ";
        ...
        <form method="GET" action="#">
          <div class="row">
            ...
            <div class="form-group">
              ...
              <input type="text" name="usr_teacher" class="form-control">
            </div>
            <div class="form-group">
          ...
          <div class="container col-4">
            ...
            <div class="form-group">
              <label>User Name</label>
              <input type="text" name="usr_student" class="form-control">
            </div>
            <div class="form-group">
              <label>Password</label>
